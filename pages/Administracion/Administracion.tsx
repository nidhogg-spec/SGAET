...
function Administracion({ usersData }: { usersData: any }) {
  ...
  const accionMostrarData = (event: any, rowData: any) => {
    const dt = datosEditables.find(
      (value: any) => value.IdUser == rowData.IdUser
    );
    setData(dt);
  ...
  const accionEliminarData = (evento: any, rowData: any) => {
    const eliminar = async () => {
      const { IdUser, Nombre, Apellido, Email, Password, TipoUsuario } =
        rowData;
        ...
        <div className={styles.titleContainer}>
          ...
          <div>
            <button
              ...
              onClick={desplegarDisplayAñadirUsuario}
            >
              Añadir usuario
            </button>
          </div>
