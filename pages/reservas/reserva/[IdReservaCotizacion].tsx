...
const estadosReservaCotizacion_array: any[] = Object.values(
  estadosReservaCotizacion
);
...
const ReservaCotizacion = ({ APIPatch }: { APIPatch: string }) => {
  ...
  const [allServiciosProductos, setAllServiciosProductos] = React.useState<
    any[]
  >([]);
  ...
  let estadosValidos = [...estadosReservaCotizacion_array];
  try {
    //@ts-ignore
    if (estadosReservaCotizacion[estado] != undefined) {
      estadosValidos = estadosReservaCotizacion_array.filter((val) => {
        if (
          //@ts-ignore
          estadosReservaCotizacion[estado]["numero"] == val["numero"]
        )
          return true;
        if (
          //@ts-ignore
          estadosReservaCotizacion[estado]["validNextStates"].includes(
            val["numero"]
          )
        )
          return true;
  ...
  const {
    ...
    defaultValues: defaultValuesServicioCotizacion,
    mode: "onBlur"
  });
  ...
  const {
    ...
    reset: resetCliente,
    handleSubmit: handleSubmitClienteCotizacion
  } = useForm({
    defaultValues: defaultValuesClienteCotizacion,
    mode: "onBlur"
  });
  ...
  const obtenerData = async () => {
    ...
    setServiciosEscogidos(
      dataReserva.reservaCotizacion?.ServicioProducto ?? []
    );
    setClienteCotizacion(dataReserva.clienteProspecto);
    dataReserva.reservaCotizacion["Estado"]
      ? setEstado(dataReserva.reservaCotizacion["Estado"])
      : setEstado(0);
        ...
        case "Dolar":
          serviciosEscogidos.map((cotiServicio: any) => {
            switch (cotiServicio["Currency"] || "Dolar") {
              ...
              case "Sol":
                tempMontoTotal +=
                  parseFloat(cotiServicio["PrecioCotiTotal"]) / cambioDolar;
                break;
            }
          });
          break;
  ...
  const handlePagadoSubmit = async () => {
    ...
    const data = {
      Adelanto: parseFloat(
        parseFloat(formData.get("Adelanto") as string).toFixed(2)
      ),
    ...
    if (
      arrayData.includes("") ||
      arrayData.includes(null) ||
      arrayData.includes(NaN)
    ) {
      alert("Faltan datos");
    } else {
  ...
  if (reservaCotizacion?.URLLlenadoPasajeros) {
    idEncripted =
      reservaCotizacion.URLLlenadoPasajeros.split("LlenadoPasajeros/");
      ...
      <div className={globalStyles.main_work_space_container}>
          ...
          {modoEdicion ? (
            <>
              <button
                ...
                onClick={handleSubmit(accionGuardar)}
              >
        ...
        <div className={styles.Estado_Input_container}>
          ...
          <select value={estado} ref={refEstado} onChange={onSelectChange}>
            {estadosValidos.map((estado: any) => (
              <option value={estado.numero}>{estado.estado}</option>
            ))}
          </select>
        </div>
        ...
        <div>
          ...
          <div>
            ...
            <div className={styles.Pasajeros_Input_container}>
              ...
              <input
                value={reservaCotizacion.URLLenadoPasajeros ?? ""}
                disabled
              ></input>
            ...
            <div className={formStyles.Modal_content}>
              <form>
                <div className={campoTextoStyles.divMadre}>
                  ...
                  <input
                    type="text"
                    {...registerCliente("NombreCompleto")}
                    disabled={!modoEdicion}
                  ></input>
                </div>
...
const TablaServicioCotizacion = (props: {
  Title: string;
        ...
        case "Dolar":
          props.CotiServicio.map((uniCotiServicio: any) => {
            switch (uniCotiServicio["Currency"] || "Dolar") {
              case "Dolar":
                tempMontoTotal += parseFloat(
                  uniCotiServicio["PrecioCotiTotal"]
                );
                break;
              case "Sol":
                tempMontoTotal +=
                  parseFloat(uniCotiServicio["PrecioCotiTotal"]) / cambioDolar;
                break;
            }
          });
          break;
  ...
  const materialTableUpdate = (cambios: any) =>
      ...
      setTimeout(() => {
        ...
        Object.entries(cambios).map((cambio: any) => {
          ...
          if (tempCotiServicio[id]["IGV"]) {
            tempCotiServicio[id]["PrecioCotiTotal"] = (
              tempNewData["Cantidad"] *
              tempNewData["PrecioCotiUnitario"] *
              1.18
            ).toFixed(2);
            tempCotiServicio[id]["PrecioConfiTotal"] = (
              tempNewData["Cantidad"] *
              tempNewData["PrecioConfiUnitario"] *
              1.18
            ).toFixed(2);
          } else {
            tempCotiServicio[id]["PrecioCotiTotal"] = (
              tempNewData["Cantidad"] * tempNewData["PrecioCotiUnitario"]
            ).toFixed(2);
            tempCotiServicio[id]["PrecioConfiTotal"] = (
              tempNewData["Cantidad"] * tempNewData["PrecioConfiUnitario"]
            ).toFixed(2);
          }
          tempCotiServicio[id]["PrecioCotiUnitario"] =
            tempNewData["PrecioCotiUnitario"];
          axios.put(
            `${props.APIPatch}/api/ServicioEscogido/CRUD/${tempCotiServicio[id]["IdServicioEscogido"]}`,
            { ServicioEscogido: tempCotiServicio[id] }
          );
        });
        props.setCotiServicio(tempCotiServicio);
        resolve();
  ...
  const materialTableDelete = (oldData: any) =>
    new Promise<void>((resolve, reject) => {
      setTimeout(async () => {
        ...
        const result = await axios.delete(
          `${props.APIPatch}/api/ServicioEscogido/CRUD/${oldData["IdServicioEscogido"]}`
        );
        resolve();
      }, 1000);
    });
  ...
  const columnasServicioAnadir: any = [
    ...
    {
      field: "PuntajeProveedor",
      title: "Puntaje del proveedor"
    },
    {
      field: "Descripcion",
      title: "Descripcion",
  ...
  const accionesServicio = [
    {
      ...
      onClick: async (event: any, rowData: any) => {
        ...
        let getAllServiciosEscogido = await axios.get(
          `${props.APIPatch}/api/reserva/DataServicio/${props.IdReservaCotizacion}`
        );
    ...
    <div>
      ...
      <div>
        ...
        <div className={styles.total_container}>
          <select
            onChange={(event) => setCurrencyTotal(event.target.value)}
