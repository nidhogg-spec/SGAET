...
import { Dialog, DialogContent, Modal, Box, Fade, Backdrop } from "@material-ui/core";
...
export default function ModalUsuarioLeer({ open, setOpen, usuario, actualizando, setActualizando }: { open: boolean, setOpen: Function, usuario: userInterface, actualizando: boolean, setActualizando: Function }) {
    ...
    const { register, handleSubmit, formState: { errors }, getValues, reset } = useForm({ defaultValues, mode: "onBlur" });
            ...
            <Dialog
                ...
                <DialogContent>
                    <div className={customStyle.postRegistro__container}>
                        ...
                        <button onClick={() => router.reload()} className={`${botones.button_border} ${botones.button} ${botones.GenericButton}`}>
                            Volver
                        </button>
                    </div>
                </DialogContent>
                    ...
                    <Box className={globalStyles.modal__MainContainer}>
                        <form action="" onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <div className={customStyle.botones_container}>
                                {!actualizando && <button className={`${botones.button} ${botones.buttonGuardar}`} type="button" onClick={() => setActualizando(true)}>Actualizar</button>}
                            </div>
                            <div>
                                <h2>Datos del Usuario</h2>
                                <div className={globalStyles.global_textInput_container}>
                                    <label>Nombre del usuario</label>
                                    <input disabled={!actualizando} type="text" {...register("Nombre", { required: true })}></input>
                                    <span className={globalStyles.global_error_message}>{errors.Nombre?.type == "required" && "El nombre del usuario es obligatorio"}</span>
                                </div>
                                <div className={globalStyles.global_textInput_container}>
                            ...
                            {actualizando && <div className={`${customStyle.botones_container} ${customStyle.botones_container_final}`}>
